...
![How to create a new resource directory](new_resource_director.png?raw=true "New Resource Directory")
...
![Adding a new menu item in editor
 view](new_menu_item.png?raw=true "Add a new menu item")

 ![Setting the menu item id](set_menu_item_id.png?raw=true "Setting the menu item id")
...
![Extracting the string resource](extract_string_resource.png?raw=true "Extract the string resource")
...
To create a toast, we call the ```makeText``` method from the ```Toast``` class which looks like this:
...
So our call to ```makeText``` replaces the TODO comment in the onOptionsItemSelected method:
